...
namespace Periodical.BL.Services
{
    ...
    {
        HostDTO GetHost(string email);
        MagazineDTO GetMagazine(int? id);
        IEnumerable<HostDTO> GetAllHosts();
        IEnumerable<HostDTO> GetAllAuthors();
        IEnumerable<MagazineDTO> GetAllMagazines();
        ...
        ClaimsIdentity Authenticate(HostDTO userDTO, string role);
        OperationSatus CreateMagazine(MagazineDTO magazineDTO, HostDTO host);
